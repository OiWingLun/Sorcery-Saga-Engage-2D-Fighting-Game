...
public class Player2Dark : MonoBehaviour
{
    ...
    public float swaysword1Damage;
    public float swaysword2Damage;
    ...
    public float swaysword1StunDuration;
    public float swaysword2StunDuration;
    private EnemyController enemyController;
    private Player2DarkAttack playerAttack;

    private PlayerController darkController;
    ...
    private void Awake()
    {
        playerAttack = GetComponent<Player2DarkAttack>();
    ...
    private void Start()
    {
        ...
        darkController = GameObject.FindGameObjectWithTag(Tags.Player_Tag).GetComponent<PlayerController>();
   ...
   private void FlipCharacter()
{
    ...
    if (darkController != null) // Replace with the actual reference if needed
    {
        enemyXPositions[validEnemyCount++] = darkController.transform.position.x;
    }
    ...
    private void HandleUserInput()
{
    ...
    if (Input.GetKeyDown(KeyCode.Keypad5) && !isAttacking && !isCrouching)
    {
        PerformKick();
    }
    ...
    if (Input.GetKeyDown(KeyCode.Keypad6) && !isCrouching)
{
    ...
    if (Input.GetKeyDown(KeyCode.Keypad4) && !isGrounded && !isAttacking)
    {
        JumpAttack1();
    }
    ...
    if (Input.GetKeyDown(KeyCode.Keypad5) && Input.GetKey(KeyCode.DownArrow) && !isAttacking && !crouchAttack1)
    {
        CrouchAttack1();  // Perform the crouch attack
    }

    if (Input.GetKeyDown(KeyCode.Keypad4) && Input.GetKey(KeyCode.DownArrow) && !isAttacking && !crouchAttack2)
    {
...
private void Special2()
{
    ...
    // Start a coroutine to handle the delayed fireball
    StartCoroutine(EndAttackAfterDelay(0.9f));
}
...
private void JumpAttack3()
{
    if (!isJumping || JumpAttacking) return;  

    JumpAttacking = true;
    myAnim.Jump(false); 
    myAnim.JumpAttack3(true);  
...
private AttackData GetAttackData(string attackTag)
{
    ...
    switch (attackTag)
    {
        case Tags.Punch_Attack_Tag:
            return new AttackData(darkController.punch1Damage, darkController.punch1StunDuration, darkController.transform.position);
        case Tags.Kick_Attack_Tag:
            return new AttackData(darkController.kick1Damage, darkController.kick1StunDuration, darkController.transform.position);
        case Tags.Special1_Attack_Tag:
            return new AttackData(darkController.special1Damage, darkController.special1StunDuration, darkController.transform.position);
        case Tags.Special2_Attack_Tag:
            return new AttackData(darkController.special2Damage, darkController.special2StunDuration, darkController.transform.position);
        case Tags.JumpAttack1_Attack_Tag:
            return new AttackData(darkController.jumpAttack1Damage, darkController.jumpAttack1StunDuration, darkController.transform.position);
        case Tags.JumpAttack2_Attack_Tag:
            return new AttackData(darkController.jumpAttack2Damage, darkController.jumpAttack2StunDuration, darkController.transform.position);
        case Tags.CrouchAttack1_Attack_Tag:
            return new AttackData(darkController.crouchAttack1Damage, darkController.crouchAttack1StunDuration, darkController.transform.position);
        case Tags.CrouchAttack2_Attack_Tag:
            return new AttackData(darkController.crouchAttack2Damage, darkController.crouchAttack2StunDuration, darkController.transform.position);
        case Tags.SwaySword1_Attack_Tag:
            return new AttackData(darkController.swaysword1Damage, darkController.swaysword1StunDuration, darkController.transform.position);
        case Tags.SwaySword2_Attack_Tag:
            return new AttackData(darkController.swaysword2Damage, darkController.swaysword2StunDuration, darkController.transform.position);
