...
{
    if (isAttacking && !jumpAttack1 && !jumpAttack2|| playerAttack.isAttacking || isBlock || isDie)
{
...
{
    if (GameController.gameController.isGameFinished || isAttacking || isDie)
        return;
...
{
    if (GameController.gameController.isGameFinished || isAttacking || playerAttack.isAttacking || isDie )
    ...
    if (Input.GetKeyDown(KeyCode.K) && !isAttacking && !isCrouching)
    {
        PerformKick();
    }
    ...
    if (Input.GetKeyDown(KeyCode.L) && !isCrouching)
{
    ...
    // Trigger special move when facing left and pressing A
    else if (!facingRight && Input.GetKey(KeyCode.D))
    {
        Special2();
    }
}
    ...
    if (Input.GetKeyDown(KeyCode.K) && Input.GetKey(KeyCode.S) && !isAttacking && !crouchAttack1)
    {
        CrouchAttack1();  // Perform the crouch attack
    }

    if (Input.GetKeyDown(KeyCode.J) && Input.GetKey(KeyCode.S) && !isAttacking && !crouchAttack2)
    {
...
private void Special3()
{
    ...
    StartCoroutine(EndAttackAfterDelay(1.4f));
}
...
{
    if (!isCrouching || isAttacking || crouchAttack1) return; 

    crouchAttack1 = true; 
    isAttacking = true; 
    myAnim.Crouch(false);
    myAnim.CrouchAttack1(true);
...
{
    if (!isCrouching || isAttacking || crouchAttack2) return; 

    crouchAttack2 = true; 
    isAttacking = true;
    myAnim.Crouch(false);
    myAnim.CrouchAttack2(true); 

    StartCoroutine(EndAttackAfterDelay(0.1f)); 
}
