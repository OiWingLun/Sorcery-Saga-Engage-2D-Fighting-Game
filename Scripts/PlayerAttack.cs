    ...
    void ComboAttack()
{
    ...
    if (Input.GetKeyDown(KeyCode.J) && canPunch)
    {
        ...
        if (currentComboState == ComboState.Punch2 || PlayerController.isJumping || PlayerController.isCrouching || PlayerController.isAttacking || PlayerController.isStunned)
        {
            return;
        }
